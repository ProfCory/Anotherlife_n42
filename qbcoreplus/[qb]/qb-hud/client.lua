...
local prevVehicleStats = { nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil }
...
local function updateVehicleHud(data)
    ...
    if shouldUpdate then
        SendNUIMessage({
            ...
            -- Retro Dash extras (safe to ignore in NUI if not used)
            retro = data[11],
            rpm = data[12],
            gear = data[13],
            engine = data[14],
            maxSpeed = data[15],
            fuelSegments = data[16],
            hideDefaultVehicleHud = data[17],
        })
    end
end
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(player) and not IsThisModelABicycle(vehicle) then
                ...
                local rawSpeed = math.ceil(GetEntitySpeed(vehicle) * speedMultiplier)
                local speedSend = rawSpeed
                if config.RetroDash and config.RetroDash.Enabled then
                    local a = tonumber(config.RetroDash.SpeedSmoothingAlpha) or 0.20
                    if a < 0.0 then a = 0.0 end
                    if a > 1.0 then a = 1.0 end
                    if smoothedSpeed <= 0 then smoothedSpeed = rawSpeed end
                    smoothedSpeed = (smoothedSpeed * (1.0 - a)) + (rawSpeed * a)
                    speedSend = math.floor(smoothedSpeed + 0.5)
                end
                updateVehicleHud({
                    show,
                    IsPauseMenuActive(),
                    ...
                    (config.RetroDash and config.RetroDash.MaxSpeed) or 180,
                    (config.RetroDash and config.RetroDash.FuelSegments) or 6,
                    (config.RetroDash and config.RetroDash.HideDefaultVehicleHud) or false,
                })
            else
...
RegisterNetEvent('aln_ui_layout:updatePos', function(id, pos)
  if id == 'vehicleDash' then
    SendNUIMessage({ action = 'setDashPos', pos = pos })
  end
end)
