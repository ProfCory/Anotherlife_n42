...
$(document).on("keydown", function () {
    switch (event.keyCode) {
        case 27: // ESC
            $.post("https://fivem-blackmarket/closeMenu", JSON.stringify());
            $(".generalSection").hide();
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (typeof userDet[i] !== "undefined") {
            if (!userDet[i].taken && userDet[i].hasCount >= element.requiredcount) {
                $.post("https://fivem-blackmarket/taskDone", JSON.stringify({ taskId: element.taskId }));
            }
            var deneme = `
            <div class="taskItem">
...
$(document).on("click", "#buyButtonMenu", function () {
    ...
    $.post(
        "https://fivem-blackmarket/buyCart",
        JSON.stringify({
            sendCart: sendCart,
