...
function openMenu()
    if openMenuSpamProtect < GetGameTimer() then 
        ...
        QBCore.Functions.TriggerCallback("fivem-blackmarket:getPlayerDetails", function(result)
...
RegisterNUICallback('taskDone', function(data, cb)
	if taskSpam < GetGameTimer() then 
		...
		QBCore.Functions.TriggerCallback("fivem-blackmarket:taskDone", function(result)
			if result then 
				SendNUIMessage({
					type = 'addEXP', 
					exp = tonumber(result),
...
RegisterNUICallback('buyCart', function(data, cb)
	if buySpam < GetGameTimer() then 
		...
		QBCore.Functions.TriggerCallback("fivem-blackmarket:buyCart", function(result)
			if result then 
				cb(true)
			else
				cb(false)
...
RegisterNetEvent('fivem-blackmarket:addtaskcount')
AddEventHandler('fivem-blackmarket:addtaskcount', function(taskId, count)
	TriggerServerEvent('fivem-blackmarket:taskCountAdd', taskId, count)
end)
