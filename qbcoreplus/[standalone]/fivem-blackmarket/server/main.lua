...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(3000)
    ExecuteSql("UPDATE blackmarket SET tasks = '"..json.encode(tasks).."', taskResetTime = (CURDATE() + INTERVAL "..AK4Y.TaskResetPeriod.." DAY) WHERE taskResetTime <= CURDATE()")
end)

QBCore.Functions.CreateCallback('fivem-blackmarket:getPlayerDetails', function(source, cb)
    local src = source
    ...
    local callbackData = {}
    local result = ExecuteSql("SELECT * FROM blackmarket WHERE citizenid = '"..citizenId.."'")
    if result[1] == nil then    
        ExecuteSql("INSERT INTO blackmarket SET citizenid = '"..citizenId.."', tasks = '"..json.encode(tasks).."', currentXP = '0', taskResetTime = (CURDATE() + INTERVAL "..AK4Y.TaskResetPeriod.." DAY)")
        callbackData = {
...
QBCore.Functions.CreateCallback('fivem-blackmarket:taskDone', function(source, cb, data)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local currentTaskId = tonumber(data.taskId)
    local result = ExecuteSql("SELECT tasks FROM blackmarket WHERE citizenid = '"..citizenId.."'")
    local correct = false
    if result[1] then 
        ...
        for k, v in pairs(tasks) do 
            if currentTaskId == v.taskId then 
                if v.hasCount >= v.requiredCount and not v.taken then 
                    ...
                    ExecuteSql("UPDATE blackmarket SET tasks = '"..json.encode(tasks).."', currentXP = currentXP + '"..addEXP.."' WHERE citizenid = '"..citizenId.."'")
                    cb(addEXP)
                else
                    cb(false)
                end
...
QBCore.Functions.CreateCallback('fivem-blackmarket:buyCart', function(source, cb, data)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
...
RegisterNetEvent('fivem-blackmarket:addXP')
AddEventHandler('fivem-blackmarket:addXP', function(amount)
    local _source = source
    local xPlayer = QBCore.Functions.GetPlayer(_source)
    local citizenId = xPlayer.PlayerData.citizenid
    local deger = tonumber(amount)
    ExecuteSql("UPDATE blackmarket SET currentXP = currentXP + '"..deger.."' WHERE citizenid = '"..citizenId.."'")
end)

RegisterNetEvent('fivem-blackmarket:taskCountAdd')
AddEventHandler('fivem-blackmarket:taskCountAdd', function(taskId, count)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local citizenId = xPlayer.PlayerData.citizenid
    local result = ExecuteSql("SELECT tasks FROM blackmarket WHERE citizenid = '"..citizenId.."'")
    if result[1] then 
        ...
        if kayit then 
            ExecuteSql("UPDATE blackmarket SET tasks = '"..json.encode(resultak4y).."' WHERE citizenid = '"..citizenId.."'")
        end
    end
end)
